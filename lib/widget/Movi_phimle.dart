...
class Listphimle extends StatelessWidget {
  const Listphimle({super.key});
    ...
    return BlocBuilder<HomeCubit, HomeState>(
      builder: (context, state) {
        if (state is HomeLoaded) {
          ...
          if (state.ListPage1 != null && state.ListPage1.isNotEmpty) {
            return SizedBox(
              height: 280,
              child: ListView.builder(
                itemCount: state.ListPage1.length,
                  ...
                  return InkWell(
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => DetailScreen(
                              url: state.ListPage1[index].slug ?? "",
                            ),
                          ),
                        );
                      } catch (e) {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        CachedNetworkImage(
                          imageUrl: _buildFullUrl(state.ListPage1[index].poster_url ?? ""),
                        
                          height: 236,
                        ),
                        Text(state.ListPage1[index].name??""),
                        Text(state.ListPage1[index].origin_name??"")
                      ],
                    ),
